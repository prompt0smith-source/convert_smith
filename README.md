...
Windows에서는 설정 패널에서 `탐색기 우클릭 메뉴 등록`을 실행할 수 있습니다. 등록 후 파일을 우클릭하고 `Convert with Convert Smith`를 선택하면 해당 파일이 Convert Smith에 추가됩니다.

우클릭 메뉴는 파일을 앱으로 불러오는 기능입니다. 원하지 않는 결과 파일 생성을 막기 위해 변환은 자동으로 시작하지 않으며, 사용자가 앱에서 변환 형식을 확인한 뒤 직접 시작해야 합니다. 이 기능은 현재 Windows에서만 지원됩니다.
