...
PDF 병합 결과 파일명을 직접 지정하지 않으면 `merged(1).pdf`, `merged(2).pdf` 방식으로 저장합니다. PDF 전체 페이지 분할 결과 파일명을 직접 지정하지 않으면 `원본파일명(1).pdf`, `원본파일명(2).pdf` 방식으로 저장합니다.

동영상 호환성 복구는 MP4/MOV/MKV/WEBM/M4V 입력을 H.264 + AAC, yuv420p, faststart MP4로 다시 인코딩합니다.

## PDF 서명 스탬프

`서명 스탬프 추가`는 PNG 또는 JPG 서명 이미지를 PDF 페이지 위에 시각적으로 삽입하는 기능입니다. 이 기능은 인증서 기반의 법적 디지털 서명이 아니며, 암호학적 위변조 방지나 법적 효력을 보장하지 않습니다.

서명된 페이지를 이미지형 페이지로 저장하는 옵션을 켜면 선택한 서명 페이지를 이미지 기반 PDF 페이지로 다시 저장하여 일반적인 PDF 편집이 어렵게 만듭니다. 이 옵션도 절대적인 위변조 방지를 의미하지 않습니다.

## Windows 탐색기 우클릭 메뉴

Windows에서는 설정 패널에서 `탐색기 우클릭 메뉴 등록`을 실행할 수 있습니다. 등록 후 탐색기 우클릭 메뉴에서 아래 항목을 사용할 수 있습니다.

- `Convert with Convert Smith`: 선택한 파일을 Convert Smith에 불러와 일반 파일 변환 화면에서 작업합니다.
- `Merge with Convert Smith`: 선택한 PDF 파일을 창 없이 백그라운드에서 병합합니다. 결과는 첫 번째 PDF가 있던 폴더에 `merged(1).pdf`, `merged(2).pdf` 방식으로 저장됩니다.
- `Split with Convert Smith`: 선택한 PDF 파일을 창 없이 백그라운드에서 페이지별 PDF로 분할합니다.

`Convert with Convert Smith`는 파일을 앱으로 불러오고 작업 모드를 준비하는 기능입니다. 원하지 않는 결과 파일 생성을 막기 위해 일반 변환은 자동으로 시작하지 않으며, 사용자가 앱에서 대상 파일과 저장 위치를 확인한 뒤 직접 시작해야 합니다. `Merge with Convert Smith`와 `Split with Convert Smith`는 빠른 PDF 작업용이므로 앱 창을 열지 않고 같은 폴더에 결과 파일을 생성합니다. 이 기능은 현재 Windows에서만 지원됩니다.

이전 버전에서 등록한 `Convert with Convert Smith` 메뉴가 PDF 파일을 열 때 JavaScript 오류를 띄우거나 `Merge with Convert Smith`가 파일 하나만 인식하면, Convert Smith 설정에서 우클릭 메뉴를 한 번 제거한 뒤 다시 등록하거나 0.1.7 이상 설치 파일로 재설치하세요.
...
직접 지정할 때는 LibreOffice 설치 폴더 안의 실제 `program\soffice.exe` 또는 `program\soffice.com`을 선택해야 합니다. 임의의 `.exe`, 바로가기, 설치 파일, 다른 Office 프로그램은 LibreOffice 실행 파일로 인정하지 않습니다.
...
- 변환 요청과 PDF 작업 요청은 main process에서 허용된 타입, 옵션, 파일 경로 형태만 통과
- 미리보기, 파일 열기, 폴더 열기는 사용자가 앱에 추가한 파일 또는 앱이 생성한 결과 파일만 허용
...
변환이 끝나면 작업 카드에 간단한 결과 리포트를 표시합니다. 리포트에는 검증 통과 여부, 원본 개수, 결과 파일 개수, 원본/결과 용량, 소요 시간이 포함됩니다.
...
PDF 편집기는 OCR을 사용하지 않습니다. PDF 안에서 실제 텍스트로 잡히는 줄과 위치 정보를 읽어 수정, 삭제, 추가한 내용을 새 PDF로 저장합니다.

현재 자체 구현은 PDF 내부 content stream의 텍스트 출력 연산자를 찾고, 단순 ANSI 또는 ToUnicode CMap으로 역매핑 가능한 경우 기존 텍스트 token을 직접 교체하거나 비워 새 PDF로 저장합니다. 내부 구조를 안전하게 확정하지 못하면 기존 텍스트 위를 가리고 새 텍스트를 얹는 방식으로 저장하지 않고 중단합니다. 원본 PDF 파일은 직접 수정하지 않습니다.

한글 텍스트 저장을 위해 `@pdf-lib/fontkit`과 로컬 Windows 폰트 매칭을 사용합니다. 로컬에 적절한 폰트가 없으면 기본 폰트로 대체되거나 일부 문자가 제한될 수 있습니다.

## 알려진 제한사항

- PDF → DOCX는 원본 레이아웃이 100% 유지되지 않을 수 있습니다.
- PDF → XLSX는 PDF에 보이는 표와 텍스트 위치를 분석해 새 엑셀 파일로 재구성합니다. 원본 엑셀의 수식, 필터, 숨김 시트, 피벗 정보는 복원되지 않을 수 있습니다.
...
- 서명 스탬프는 시각적 이미지 삽입 기능이며 인증서 기반 법적 디지털 서명이 아닙니다.
- 이미지형 페이지 저장 옵션은 일반적인 PDF 편집을 어렵게 만들지만 절대적인 위변조 방지를 보장하지 않습니다.
- Windows 탐색기 우클릭 메뉴는 Windows 전용이며 변환을 자동으로 시작하지 않습니다.
